...
- **A poll reads behind the last one.** Statuspage is served from a CDN, so a poll can be handed a copy of an incident older than the one already folded in — typically missing only its newest update, which is the one a slightly stale copy is most likely not to have. That is indistinguishable, payload-to-payload, from the operator having deleted it, and treating it as a deletion strikes a live update through permanently. Two rules, because either alone leaves a hole: an incident whose `updated_at` is **behind** the stored `src_updated_at` is dropped whole rather than diffed, and an update marked deleted that turns up in a later payload is **restored**. The first is prevention and the second is repair — Statuspage has no un-delete, so a reappearance is near-always a deletion of ours that should not have happened, and a soft delete is only cheap if something can undo it.
